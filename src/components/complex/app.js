  ...
  render() {
    return (
      <BootstrapTable data={ jobs }
        ...
        dataAlign='center'
        width='50'
        dataSort isKey autoValue>ID</TableHeaderColumn>

        <TableHeaderColumn width='120' dataField='name' className='good' dataSort
          editable={ { type: 'textarea', validator: nameValidator } }>Job Name</TableHeaderColumn>

        <TableHeaderColumn dataField='priority' width='100'dataSort dataFormat={ priorityFormatter }
          editable={ {
            type: 'select',
            options: { values: [ 'A', 'B', 'C', 'D' ] },
            validator: priorityValidator } }>Priority</TableHeaderColumn>
